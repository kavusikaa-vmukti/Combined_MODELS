...
while True:
    ...
    for result in results:
        for box in result.boxes:
            ...
            if conf > 0.5:
                ...
                y2_scaled = int(y2 * (screen_height / frame.shape[0]))
                ...
                draw_label_with_background(frame, label, x1, y1 - 10, color, font_scale=0.6, thickness=1)
    ...
    for person_bbox in persons:
        ...
        if not is_vest:  # Change color if no vest detected
            color = (0, 0, 255)

        cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
        draw_label_with_background(frame, label, x1, label_y_offset, color, font_scale=0.6, thickness=1)
